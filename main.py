...
def altimetry_plot(topography,
    ...
    fig.update_layout(xaxis2=dict(showgrid=False,visible=False,range=[-5,NUM_FRAMES],fixedrange=True),yaxis2=dict(showgrid=False,visible=False,range=[0,1.5],fixedrange=True))
    fig.update_layout(xaxis=dict(showgrid=False,visible=False,range=[-1,1],fixedrange=True),yaxis=dict(showgrid=False,visible=False,range=[-0.1,4.35],fixedrange=True))
...
To run the model, press the **PLAY** button below. This emits pulses from the satellite towards the surface, decreasing exponentially in power over emission angle, which are then reflected, and subsequently captured by the satellite. On the right, the waveform records the recieved power with respect to time, with the addition of simulated guassian noise. For both the 2D view and the waveform, a range window is also shown, highlighted in green.
